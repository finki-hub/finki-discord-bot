...
const logEvent = async (event: UsageEvent) => {
  ...
  try {
    ...
    logger.warn(`Failed logging analytics event\n${String(error)}`);
...
export const logCommandEvent = async (
  ...
  const { basePayload, eventType, logContext = true } = options;
  if (!interaction.channel?.isTextBased()) {
    return;
  }

  logger.info(`Logging command ${interaction.commandName} in analytics...`);
